...
exports.BinaryTypeBuilder = exports.BinaryField = exports.BinarySchema = void 0;
const Util = require("util");
const Long = require("long");
const ObjectType_1 = require("../ObjectType");
const BinaryTypeStorage_1 = require("./BinaryTypeStorage");
const BinaryUtils_1 = require("./BinaryUtils");
const BinaryCommunicator_1 = require("./BinaryCommunicator");
...
class BinaryType {
        ...
        for (let field of binaryType.fields) {
            ...
            if (this.hasField(fieldId)) {
                if (this.getField(fieldId).typeCode !== field.typeCode) {
                    throw Errors_1.IgniteClientError.serializationError(true, Util.format('type conflict for field "%s" of complex object type "%s"', field.name, this._name));
                }
            }
            else {
                this.setField(field);
    ...
    static _calculateId(name) {
        return BinaryUtils_1.default.strHashCodeLowerCase(name);
    }
    _write(buffer) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            BinaryCommunicator_1.default.writeString(buffer, null);
            // fields count
            buffer.writeInteger(this._fields.size);
            // fields
            for (let field of this._fields.values()) {
                yield field._write(buffer);
            }
            yield this._writeEnum(buffer);
            // schemas count
            buffer.writeInteger(this._schemas.size);
            for (let schema of this._schemas.values()) {
                yield schema._write(buffer);
            }
        });
    }
    _writeEnum(buffer) {
    ...
    _read(buffer) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // type name
            this._name = BinaryCommunicator_1.default.readString(buffer);
            // affinity key field name
            BinaryCommunicator_1.default.readString(buffer);
            // fields count
            ...
            for (let i = 0; i < fieldsCount; i++) {
                ...
                yield field._read(buffer);
                this.setField(field);
            }
            yield this._readEnum(buffer);
            // schemas count
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this._isEnum) {
                ...
                for (let i = 0; i < valuesCount; i++) {
                    this._enumValues[i] = [BinaryCommunicator_1.default.readString(buffer), buffer.readInteger()];
                }
            }
        });
    }
...
class BinaryTypeBuilder {
    ...
    static fromTypeId(communicator, typeId, schemaId) {
        return __awaiter(this, void 0, void 0, function* () {
            let result = new BinaryTypeBuilder();
            let type = yield communicator.typeStorage.getType(typeId, schemaId);
            if (type) {
                result._type = type;
                if (schemaId !== null) {
                    result._schema = type.getSchema(schemaId);
                    if (!result._schema) {
                        throw Errors_1.IgniteClientError.serializationError(false, Util.format('schema id "%d" specified for complex object of type "%s" not found', schemaId, type.name));
                    }
    ...
    static fromObject(jsObject, complexObjectType = null) {
        if (complexObjectType) {
            ...
            result._fromComplexObjectType(new ObjectType_1.ComplexObjectType(jsObject), jsObject);
            return result;
        }
    }
    static fromComplexObjectType(complexObjectType, jsObject) {
        let result = new BinaryTypeBuilder();
        const typeInfo = BinaryTypeStorage_1.default.getByComplexObjectType(complexObjectType);
        if (typeInfo) {
            ...
            BinaryTypeStorage_1.default.setByComplexObjectType(complexObjectType, result._type, result._schema);
        }
        return result;
    }
    getTypeId() {
    ...
    finalize(communicator) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield communicator.typeStorage.addType(this._type, this._schema);
        });
    }
    ...
    _setFields(complexObjectType, objectTemplate, jsObject) {
        let fieldType;
        for (let fieldName of BinaryUtils_1.default.getJsObjectFieldNames(objectTemplate)) {
            fieldType = complexObjectType._getFieldType(fieldName);
            if (!fieldType && jsObject[fieldName]) {
                fieldType = BinaryUtils_1.default.calcObjectType(jsObject[fieldName]);
            }
            this.setField(fieldName, BinaryUtils_1.default.getTypeCode(fieldType));
        }
    }
}
exports.BinaryTypeBuilder = BinaryTypeBuilder;
