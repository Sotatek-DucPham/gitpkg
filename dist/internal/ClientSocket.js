...
class ClientSocket {
    constructor(endpoint, config, communicator, onSocketDisconnect, onAffinityTopologyChange) {
        ArgumentChecker_1.default.notEmpty(endpoint, 'endpoints');
    ...
    _connectSocket(handshakeRequest) {
        ...
        const options = Object.assign({}, this._config.options, { host: this._host, port: this._port, version: this._version });
    ...
    _processResponse(message) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this._buffer) {
                ...
                this._buffer = MessageBuffer_1.default.from(message, 0);
            }
            while (this._buffer && this._offset < this._buffer.length) {
                const buffer = this._buffer;
                // Response length
                const length = buffer.readInteger() + BinaryUtils_1.default.getSize(BinaryUtils_1.default.TYPE_CODE.INTEGER);
                ...
                if (this._requests.has(requestId)) {
                    ...
                    if (isHandshake) {
                        yield this._finalizeHandshake(buffer, request);
                    }
                    else {
                        yield this._finalizeResponse(buffer, request);
                    }
                }
                else {
                    throw Errors_1.IgniteClientError.internalError('Invalid response id: ' + requestId);
                }
            }
        });
    }
    _finalizeHandshake(buffer, request) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!isSuccess) {
                ...
                // Error message
                const errMessage = BinaryCommunicator_1.default.readString(buffer);
                if (!this._protocolVersion.equals(serverVersion)) {
                    if (!this._isSupportedVersion(serverVersion) ||
                        ...
                        request.reject(new Errors_1.OperationError(Util.format('Protocol version mismatch: client %s / server %s. Server details: %s', this._protocolVersion.toString(), serverVersion.toString(), errMessage)));
                        this._disconnect();
                    }
                    else {
                    ...
                    request.reject(new Errors_1.OperationError(errMessage));
                    this._disconnect();
                }
            }
            else {
                if (this._protocolVersion.compareTo(PROTOCOL_VERSION_1_4_0) >= 0) {
                    this._nodeUuid = yield this._communicator.readObject(buffer, BinaryUtils_1.default.TYPE_CODE.UUID);
                }
    ...
    _finalizeResponse(buffer, request) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this._protocolVersion.compareTo(PROTOCOL_VERSION_1_4_0) < 0) {
                ...
                if (flags & FLAG_TOPOLOGY_CHANGED) {
                    const newVersion = new PartitionAwarenessUtils_1.AffinityTopologyVersion(buffer);
                    yield this._onAffinityTopologyChange(newVersion);
                }
                if (!isSuccess) {
                    statusCode = buffer.readInteger();
                }
    ...
    _handshakePayloadWriter(payload) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this._config.userName) {
                BinaryCommunicator_1.default.writeString(payload, this._config.userName);
                BinaryCommunicator_1.default.writeString(payload, this._config.password);
            }
        });
    }
    ...
    _disconnect(close = true, callOnDisconnect = true) {
        ...
        this._requests.forEach((request, id) => {
            request.reject(new Errors_1.LostConnectionError(this._error));
        ...
        if (colonCnt > 1) {
            ...
            if (this._host.startsWith('[') || this._host.endsWith(']')) {
                if (this._host.startsWith('[') && this._host.endsWith(']')) {
                    ...
                    throw Errors_1.IgniteClientError.illegalArgumentError('Incorrect endpoint format: ' + endpoint);
                }
            }
        }
        else {
    ...
    _logMessage(requestId, isRequest, message) {
        if (Logger_1.default.debug) {
            Logger_1.default.logDebug((isRequest ? 'Request: ' : 'Response: ') + requestId);
            Logger_1.default.logDebug('[' + [...message] + ']');
        }
    }
}
...
class Request {
    ...
    getMessage() {
        return __awaiter(this, void 0, void 0, function* () {
            const message = new MessageBuffer_1.default();
            // Skip message length
            const messageStartPos = BinaryUtils_1.default.getSize(BinaryUtils_1.default.TYPE_CODE.INTEGER);
